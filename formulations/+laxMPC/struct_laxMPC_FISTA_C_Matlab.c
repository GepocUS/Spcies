...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    if(nlhs == 0){
        mexErrMsgIdAndTxt("Spcies:laxMPC:nlhs:number", "At least one output is required");
    }

    // Check that x0 is of the correct dimension
    if( !mxIsDouble(prhs[0]) || mxGetNumberOfElements(prhs[0]) != nn_ ){
        mexErrMsgIdAndTxt("Spcies:laxMPC:nrhs:x0", "x0 must be of dimension %%d", nn_);
    }
    // Check that xr is of the correct dimension
    if( !mxIsDouble(prhs[1]) || mxGetNumberOfElements(prhs[1]) != nn_ ){
        mexErrMsgIdAndTxt("Spcies:laxMPC:nrhs:xr", "xr must be of dimension %%d", nn_);
    }
    ...
    z_pt = mxCreateDoubleMatrix(NN_*nm_, 1, mxREAL);
    ...
    // Call solver
    #if TIME_VARYING == 1
    laxMPC_FISTA(x0, xr, ur, A, B, Q, R, LB, UB, u_opt, &k, &e_flag, &sol);
    #else
    laxMPC_FISTA(x0, xr, ur, u_opt, &k, &e_flag, &sol);
    #endif
