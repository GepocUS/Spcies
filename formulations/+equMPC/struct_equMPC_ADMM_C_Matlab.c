...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    z_pt = mxCreateDoubleMatrix(NN_*nm_-nn_, 1, mxREAL);
    v_pt = mxCreateDoubleMatrix(NN_*nm_-nn_, 1, mxREAL);
    lambda_pt = mxCreateDoubleMatrix(NN_*nm_-nn_, 1, mxREAL);
    ...
    // Call solver
    #if TIME_VARYING == 1
    equMPC_ADMM(x0, xr, ur, A, B, Q, R, LB, UB, u_opt, &k, &e_flag, &sol);
    #else
    equMPC_ADMM(x0, xr, ur, u_opt, &k, &e_flag, &sol);
    #endif
    ...
    *run_time_out = sol.run_time;
    ...
    for (unsigned int i = 0; i < NN_*nm_-nn_; i++) {
        z_out[i] = sol.z[i];
        v_out[i] = sol.v[i];
