...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    if(nrhs != 3){
        mexErrMsgIdAndTxt("Spcies:MPCT_ess:nrhs:number",
                          "Not enough inputs");
    }

    // Check number of outputs
    if(nlhs == 0){
        mexErrMsgIdAndTxt("Spcies:MPCT_ess:nlhs:number",
    ...
    z_pt = mxCreateDoubleMatrix(NN_*dnm_, 1, mxREAL);
    v_pt = mxCreateDoubleMatrix(NN_*dnm_, 1, mxREAL);
    lambda_pt = mxCreateDoubleMatrix(NN_*dnm_, 1, mxREAL);
    ...
    // Call solver
    MPCT_ADMM_cs(x0, xr, ur, u_opt, &k, &e_flag, &sol);
