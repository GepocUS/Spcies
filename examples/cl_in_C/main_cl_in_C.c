...
/*
    ...
    - Dimensions of the system and MPC: nn_ (dimension of system state), mm_ (dimension of control
     input), nm_ (nn_ + mm_), NN_ (prediction horizon), etc.
    ...
    INPUTS:
    - double *x0_in: A pointer to an array x[nn_] which contains the current system state.
    - double *xr_in: A pointer to an array xr[nn_] which contains the state reference.
    - double *ur_in: A pointer to an array ur[mm_] which contains the control input reference.
    OUTPUTS:
    - double *u_opt: A pointer to an array u[mm_] where the MPC control action will be returned.
    - int *k_in: A pointer to an int k where the number of iterations taken by the solver will be returned.
...
/*
    ...
    gcc main_cl_in_C.c myMPCsolver.c
...
int main() {
    ...
    double x[nn_] = {0.0}; // For storing the system state
    double u[mm_] = {0.0}; // For storing the control input
    double xr[nn_] = {0.25, 0.25, 0.25, 0.0, 0.0, 0.0}; // State reference
    double ur[mm_] = {0.5, 0.5}; // Control input reference
    ...
    for (int t = 1; t <= num_iter; t++) {
        ...
        // Simulate system
        double x_aux[nn_] = {0.0}; // Auxiliary array for computing the successor state
        for (int i = 0; i < nn_; i++) {
            for (int j = 0; j < nn_; j++) {
                x_aux[i] += AB[i][j] * x[j];
            }
            for (int j = 0; j < mm_; j++) {
                x_aux[i] += AB[i][nn_+j] * u[j];
            }
        }
        for (int i = 0; i < nn_; i++) {
            x[i] = x_aux[i];
