...
% We will need a system. We will consider a system composed of three masses
...
% The easiest way to set the options is to save them into a structure with the
% appropriate field names.
% We will set some options. For a full list of the available options, please
% consult the laxMPC's documentation.
...
options.tol = 0.001; % Exit tolerance of the solver

% Next, we can set some of the options of the toolbox, such as the name of the
% mex file that will be generated.
spcies_options.save_name = 'lax_solver';

% We now generate the mex file by calling the following function:
Spcies_gen_controller('sys', sys, 'param', param, 'options', options,...
    'spcies_options', spcies_options, 'target', 'Matlab');

% Notice that we are indicating the 'target' as 'Matlab', which is telling
% the function to generate a mex file. This was not strictly needed, since
% the default value for the 'target' is set to Matlab.
% For plain C code, set the 'target' argument to 'C'.

% Another way of passing the target to the function would have been to include
% it in the spcies_options structure in the field 'target'.

% A mex file should have been saved into the working directory if a Matlab
