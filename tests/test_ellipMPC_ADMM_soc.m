...
% for the version in which the terminal constraint
% is imposed using a SOC.

function [gap, exit] = test_ellipMPC_ADMM_soc(sys, status)

    % Solver options
    solver_options.rho = 15;
    solver_options.sigma = 10;
    ...
    'platform', 'Matlab', 'formulation', 'ellipMPC', 'method', 'ADMM', 'submethod', 'soc');

    % Solve using the sparse solver
    [u_s, k_s, e_s, sol_s] = ellipMPC(status.x, status.xr, status.ur, param.r);
    
    % Solse using the non-sparse solver
    [u_ns, k_ns, e_ns, sol_ns] = spcies_ellipMPC_ADMM_soc_solver(status.x, status.xr, status.ur, 'sys', sys,...
    ...
    gap.opt = norm(sol_s.z(1:end-1) - z_opt, Inf);                   
