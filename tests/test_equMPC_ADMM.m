%% Test for the equMPC formulation using ADMM

function [gap, exit] = test_equMPC_ADMM(sys, status)
...
%     sys.UBu = kron(ones(1, param.N), sys.UBu);
    ...
    'platform', 'Matlab', 'formulation', 'equMPC', 'method', 'ADMM');

    % Solve using the sparse solver
    [u_s, k_s, e_s, sol_s] = equMPC(status.x, status.xr, status.ur);
    
    % Solse using the non-sparse solver
    [u_ns, k_ns, e_ns, sol_ns] = spcies_equMPC_ADMM_solver(status.x, status.xr, status.ur, 'sys', sys,...
