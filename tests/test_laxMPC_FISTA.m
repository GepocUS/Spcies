%% Test for the laxMPC formulation using FISTA

function [gap, exit] = test_laxMPC_FISTA(sys, status)
...
%     sys.UBu = kron(ones(1, param.N+1), sys.UBu);
    ...
    [u_ns, k_ns, e_ns, sol_ns] = spcies_laxMPC_FISTA_solver(status.x, status.xr, status.ur, 'sys', sys,...
