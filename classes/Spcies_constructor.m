        ...
        function self = construct(self, options)
            ...
            for i = 1:numel(fn)
                ...
                if ~ self.files.(name).override
                    self.files.(name).dir.name = sp_utils.find_unused_file_name(self.files.(name).dir);
                end
                ...
                for j = 1:num_data
                    ...
                    data_string = sp_utils.declare_variables(self.files.(name).data{j, 2}, options);
                ...
                file_text.(name) = sp_utils.add_notice(file_text.(name), options); % Notice saying that is was generated by Spcies
            ...
            for j = 1:num_data
                ...
                % Process the data into a string
                data_string = sp_utils.declare_variables(self.data{j, 2}, options);
            ...
            for i = 1:numel(fn)
                ...
                % Open file
                file_pointers.(fn{i}) = fopen(sp_utils.get_full_path(self.files.(fn{i}).dir), 'wt');
