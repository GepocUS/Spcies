...
% This version uses the "diamond" sets.
% It also uses the ADMM splitting method (\hat z, \hat s) = (z, s).
%
% This function is part of Spcies: https://github.com/GepocUS/Spcies
% 

function [u, k, e_flag, Hist] = spcies_HMPC_SADMM_split_solver(x0, xr, ur, varargin)
    ...
    par.FunctionName = 'spcies_HMPC_SADMM_split_solver';
    ...
    options = Spcies_options('formulation', 'HMPC', 'method', 'SADMM', 'submethod', 'split', 'options', options);
    ...
    var = HMPC.compute_HMPC_SADMM_split_ingredients(controller, options);
    N = var.N;
    n = var.n;
    m = var.m;
    n_y = var.n_y;
...
%             rhs = sp_utils.LDLsolve(var.L_CSC.val, var.L_CSC.row, var.L_CSC.col, var.Dinv, var.Pldl'*[-q_hat; bh]);
            rhs = sp_utils.LDLsolve(var.L_CSC.val, var.L_CSC.row, var.L_CSC.col, var.Dinv, [-q_hat; bh]);
        end
        ...
        if options.solver.box_constraints
            ...
            if options.solver.use_soc
                ...
                for j = 1:n+m
                    s(3*(j-1) + (1:3)) = sp_utils.proj_D(s_proj(3*(j-1) + (1:3)), var.LBy(j), var.UBy(j));
                end
            end
            ...
            if options.solver.use_soc
                ...
                for j = 1:n_y
                    s(N*n_y + 3*(j-1) + (1:3)) = sp_utils.proj_D(s_proj(N*n_y + 3*(j-1) + (1:3)), var.LBy(j), var.UBy(j));
                end
            end
