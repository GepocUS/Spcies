%% spcies_MPCT_ADMM_band_solver - Solver for the MPCT formulation using the ADMM algorithm
%
...
%
% [u, k, e_flag, Hist] = spcies_MPCT_ADMM_band_solver(x0, xr, ur, 'name', value, 'name', ...)
%
% INPUTS:
...
function [u, k, e_flag, Hist] = spcies_MPCT_ADMM_band_solver(x0, xr, ur, varargin)
    ...
    par.FunctionName = 'spcies_MPCT_ADMM_band_solver';
    ...
    var = compute_MPCT_ADMM_band_ingredients(controller, options, []);
    N = var.N;
